...
ignore_stations = ['NONE'] # ['LAMB', 'SMAL', 'THOR']

stations = pd.read_csv('/space/wp280/CCFRFR/frfr_stations.csv')

rayleigh_pat = '/space/wp280/SCRIPTS/other/output_ZZ.csv'
love_pat = '/space/wp280/SCRIPTS/other/output_TT.csv' 

rayleigh_g_pat = '/space/wp280/SCRIPTS/other/output_ZZ_G.csv'
love_g_pat = '/space/wp280/SCRIPTS/other/output_TT_G.csv' 

############

data_pd = pd.read_csv(rayleigh_pat)
rayleigh_data = {}

for _, row in data_pd.iterrows():
    key = row['sta1net'] + '_' + row['sta1sta'] + '_' + row['sta2net'] + '_' + row['sta2sta']
    if isinstance(row['curve'], str):
        curve = ast.literal_eval(row['curve'])
    else:
        continue
    periods = [k[0] for k in curve]
    vels    = [k[1]/1000 for k in curve]
                
    rayleigh_data[key] = [periods, vels]

data_pd = pd.read_csv(love_pat)
love_data = {}
...
with open('surfdata.dat','w') as fout:
    ...
    print(UniqPeriods, len(UniqPeriods))
